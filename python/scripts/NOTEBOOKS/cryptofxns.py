...
def load_centrom_locs(filepath='../FILES/JEC21_cens.tsv',cent_col=['Name','Start','Stop']):
    """
...
def draw_QTLs(df, phenolabel, chrlist=None, clens=None, ylabel = "$-\log_{10}\, p$",
              colors=colors, alpha=0.9,mk = '.', ls = ' ',
              mylines=None,fprefix=None,lw =.3,yscale = None, 
              Close = False, centros = None, DPI = 600):
    if chrlist is None:
        chrlist = df.Chrom.unique().tolist()
    if clens is None:
        clens = chromosome_lengths
    ...
    for i, chrom in enumerate(chrlist):
        ...
        if centros is not None: ## Centromeres
            plt.broken_barh([(centros.ix[chrom,0],centros.ix[chrom,1] - centros.ix[chrom,0])], 
                (centros.ix[chrom,2], centros.ix[chrom,3]), facecolors='black')
            if i == 3 : ## Mat Locus
                plt.broken_barh([(mats,matst - mats)], 
                (centros.ix[chrom,2], centros.ix[chrom,3]), facecolors='green')
...
def draw_all_QTLs(df, phenolabels, chrlist=None, clens=None, ylabel = "$-\log_{10}\, p$",
              colors=colors, alpha=0.9,mk = '.', ls = ' ',
              mylines=None,fprefix=None,lw =.3,yscale = None, 
              Close = False, centros = None, DPI = 600):
    if chrlist is None:
    ...
    chrom_midpts = 0.5 *  np.sum(np.array(zip([0]+list(np.cumsum(clens)),
                                      np.cumsum(clens))),axis=1)
    fig, ax1 = plt.subplots(len(phenolabels),1,figsize=(16,16), dpi=150,sharex=True,sharey=True)
    for k,phenolabel in enumerate(phenolabels):
        clrcycle = cycle(colors)
        for i, chrom in enumerate(chrlist):
            sub = df.query("Chrom == '%s'" % chrom)
            clr = clrcycle.next()
            ax1[k].plot(sub.Pos + offset_dict[chrom], sub[phenolabel], 
            marker = mk, linestyle=ls, color=clr, alpha=alpha)
        
        ax1[k].set_ylabel(ylabel+'\n'+phenolabel, fontsize=14)
        ax1[k].set_xticks(chrom_midpts)
        ax1[k].set_xticklabels([str(i) for i in range(1, len(clens)+1)])
        if yscale is not None:
            ax1[k].set_ylim(0,yscale)
        if mylines is not None:
            for they in mylines:
                plt.hlines(they,min(sub.Pos),max(sub.Pos),linestyle='--',linewidth=lw)
    ax1[k].set_xlabel("Chromosomes")
    plt.tight_layout();
    plt.subplots_adjust(top=0.95)
    if fprefix is not None:
        fig.savefig(fprefix + '-{}.pdf'.format(phenolabel), dpi=DPI)
...
def IMIandCON(df,chrlist=None,chrlab='Chrom',poslab='Pos',tc=4,unit_scale = 1e-6):
    ...
    for chrom in chrlist:
        ...
        temp = sub.iloc[:sub_in[-1],tc:].reset_index(drop=True) - sub.iloc[1:,tc:].reset_index(drop=True)
...
def crypto_kruskal(site,pheno):
    try:
        pval = abs(-np.log10(ss.kruskal(np.array(pheno)[np.array(site) == 0],np.array(pheno)[np.array(site)==1],nan_policy='omit')[1]))
    except ValueError:
        pval = np.nan
    return pval